    ...
    bool shadowed = false;
        ...
        attenuation *= tex2D(_LightTextureB0, (dot(lightVec, lightVec) * _LightPos.w).rr).UNITY_ATTEN_CHANNEL;
            ...
            attenuation *= uvCookie.w < 0;
                ...
                shadowAttenuation *= texCUBEbias(_LightTexture0, float4(uvCookie, -8)).w;
            #endif
...
Interpolators VertexProgram (VertexData v)
{
    ...
    i.ray = lerp(UnityObjectToViewPos(v.normal) * float3(-1, -1, 1), v.normal, _LightAsQuad);
    return i;
}
...
float4 FragmentProgram (Interpolators i) : SV_Target
{
    ...
    UnityLight light = CreateLight(i.uv, worldPos, viewPos.z);
    //light.color = 0;
    //light.dir = 0;
